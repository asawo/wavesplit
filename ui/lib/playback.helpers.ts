import type { StemStateMap, StemKey } from "./types";
...
export function makeWaveformBars(seed: string, count: number): number[] {
  let s = hashStr(seed);
...
export function extractWaveform(
  ...
  const result = new Array<number>(numPoints);
...
export function waveformGradientId(trackId: string, stemKey: string): string {
